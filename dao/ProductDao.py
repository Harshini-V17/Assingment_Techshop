...
class ProductDao:
    ...
    def get_product_details(self, product_id):
        try:
            ...
            sql_query = "SELECT * FROM Products WHERE ProductID = ?"
    ...
    def update_product_info(self, product_id, price=None, description=None):
        try:
            ...
            if updates:
                ...
                sql_query += ", ".join([f"{key} = ?" for key in updates.keys()])
                sql_query += " WHERE ProductID = ?"
    ...
    def is_product_in_stock(self, product_id):
        try:
            connection = DBConnUtil.getConnection()
            cursor = connection.cursor()
            sql_query = "SELECT QuantityInStock FROM Inventory WHERE ProductID = ?"
            cursor.execute(sql_query, (product_id,))
    ...
    def insert_product(self, product_name, description, price):
        try:
            ...
            sql_query = "INSERT INTO Products (ProductName, Description, Price) VALUES (?, ?, ?)"
    ...
    def get_product_recommendations(self, category):
        try:
            ...
            sql_query = """
                ...
                FROM Products
                WHERE Category = ?
                ORDER BY NEWID()  -- Randomly order results
                OFFSET 0 ROWS
