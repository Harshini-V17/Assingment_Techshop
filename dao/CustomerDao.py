...
class CustomerDao():
    def calculateTotalOrders(self, customerId):
        try:
            ...
            if connection is None:  # Check if the connection was successful
                print("Failed to connect to the database.")
                return
            cursor = connection.cursor()
            sqlQuery = "SELECT COUNT(*) FROM Orders WHERE CustomerID = ?"
    ...
    def updateCustomerInfo(self, customerId, email=None, phone=None, address=None):
        try:
            ...
            if updates:
                ...
                sqlQuery += ", ".join([f"{key} = ?" for key in updates.keys()])
                sqlQuery += " WHERE CustomerID = ?"
    ...
    def insertCustomer(self, firstName, lastName, email, phone, address):
        try:
            ...
            sqlQuery = "INSERT INTO Customers (FirstName, LastName, Email, Phone, Address) VALUES (?, ?, ?, ?, ?)"
