...
class InventoryDao():
    ...
    def get_quantity_in_stock(self, inventory_id):
        try:
            connection = DBConnUtil.getConnection()
            cursor = connection.cursor()
            sql_query = "SELECT QuantityInStock FROM Inventory WHERE InventoryID = ?"
            cursor.execute(sql_query, (inventory_id,))
    ...
    def update_stock_quantity(self, inventory_id, new_quantity):
        try:
            ...
            sql_query = "UPDATE Inventory SET QuantityInStock = ? WHERE InventoryID = ?"
    ...
    def insert_inventory(self, product_id, quantity_in_stock, last_stock_update):
        try:
            ...
            sql_query = "INSERT INTO Inventory (ProductID, QuantityInStock, LastStockUpdate) VALUES (?, ?, ?)"
            cursor.execute(sql_query, (new_inventory.product_id, new_inventory.quantity_in_stock, new_inventory.last_stock_update))
            connection.commit()
