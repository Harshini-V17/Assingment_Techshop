...
class OrderDetailsDao:
    ...
    def update_quantity(self, order_detail_id, new_quantity):
        try:
            ...
            update_quantity_query = "UPDATE OrderDetails SET Quantity = ? WHERE OrderDetailID = ?"
            ...
            update_total_amount_query = "UPDATE Orders SET TotalAmount = ? WHERE OrderID = (SELECT OrderID FROM OrderDetails WHERE OrderDetailID = ?)"
    ...
    def add_discount(self, order_detail_id, discount_percentage):
        try:
            ...
            if rows:
                ...
                update_query = "UPDATE Orders SET TotalAmount = ? WHERE OrderID = (SELECT OrderID FROM OrderDetails WHERE OrderDetailID = ?)"
                cursor.execute(update_query, (total_after_discount, order_detail_id))

                print("Discount applied successfully.\n New total price:", total_after_discount)
            else:
    ...
    def insert_order_detail(self, order_id, product_id, quantity):
        try:
            ...
            sql_query = "INSERT INTO OrderDetails (OrderID, ProductID, Quantity) VALUES (?, ?, ?)"
